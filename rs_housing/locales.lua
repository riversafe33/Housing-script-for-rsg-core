Locales = {}

Locales = {
    ["MENU_FURNITURE"]                             = "Furniture",
    ["MENU_FURNITURE_DESCRIPTION"]                 = "Manage the property´s furniture.",
    ["TITLE"]                                      = "Placement",
    ["SPEED"]                                      = "Speed",
    ["POSITIONGROUP"]                              = "Position",
    ["ELEVATIONGROUP"]                             = "Elevation",
    ["ROTATIONGROUP"]                              = "Rotation",
    ["FORWARD"]                                    = "Forward",
    ["BACKWARD"]                                   = "Backward",
    ["LEFT"]                                       = "Left",
    ["RIGHT"]                                      = "Right",
    ["UP"]                                         = "Up",
    ["DOWN"]                                       = "Down",
    ["ROTPLUS"]                                    = "Rotate +",
    ["ROTMINUS"]                                   = "Rotate -",
    ["CONFIRM"]                                    = "Confirm",
    ["CANCEL"]                                     = "Cancel",
    ["FOURTOCANCEL"]                               = "Press ~e~4~q~ to cancel and exit placement",
    ["GITEMPLACE"]                                 = "Press ~e~G~q~ to place the object",
    ["YESPLACE"]                                   = "Yes",
    ["NOPLACE"]                                    = "No",
    ["USEDMENU"]                                   = "Use the menu to move the object",
    ["HOUSING_NOTI"]                               = "Property",
    ["FURNITURE_NO_ACCESS"]                        = "You don't have permission to place furniture in this property.",
    ["FURNITURE_LIMIT_REACHED"]                    = "Furniture limit reached.",
    ["FURNITURE_NOT_ENOUGH_MONEY"]                 = "You don´t have enough money.",
    ["FURNITURE_SAVE_ERROR"]                       = "Error saving the furniture.",
    ["FURNITURE_PLACED"]                           = "Furniture placed successfully.",
    ["FURNITURE_NO_ITEM"]                          = "You don´t have that furniture in your inventory.",
    ["FURNITURE_REMOVE_ITEM_ERROR"]                = "Could not remove the furniture from your inventory.",
    ["FURNITURE_NOT_FOUND"]                        = "Furniture not found.",
    ["FURNITURE_SOLD"]                             = "You sold",
    ["FOR"]                                        = "for",
    ["FURNITURE_REMOVED"]                          = "Furniture removed:",
    ["FURNITURE_INVENTORY_FULL"]                   = "Inventory full, the furniture was lost.",
    ["MENU_FURNITURE_BUY"]                         = "Buy Furniture",
    ["MENU_FURNITURE_SELL"]                        = "Sell Furniture",
    ["MENU_FURNITURE_PLACE"]                       = "Place Furniture",
    ["MENU_FURNITURE_REMOVE"]                      = "Remove Furniture",
    ["MENU_FURNITURE_SUBTEXT_PROP"]                = "Property:",
    ["MENU_FURNITURE_SUBTEXT_RANGE"]               = "Range:",
    ["CAMERA"]                                     = "Hold left mouse button to move the camera",
    ["PROPERTY_ON_SALE_BLIP"]                      = "Property On Sale",
    ["PROPERTY_ON_SALE_BLIP_ID"]                   = "Property On Sale (#%s)",
    ["PROPERTY_OWNED_BLIP"]                        = "Owned Property",
    ["PROPERTY_PROMPT_FOOTER"]                     = "Cost: ~t6~%s $ ~q~| Tax: ~t6~$%s ~q~",
    ["PROPERTY_PROMPT_FOOTER_NO_TAX"]              = "Cost: ~t6~%s $ ~q~",
    ["PROPERTY_INVENTORY"]                         = "Storage Id -",
    ["PROPERTY_TELEPORT_PROMPT_FOOTER_PRIMARY"]    = "Enter Property",
    ["PROPERTY_TELEPORT_PROMPT_FOOTER_SECONDARY"]  = "Leave Property",
    ["PROPERTY_PROMPT_MENU_OPEN_FOOTER"]           = "Property Menu Actions",
    ["PROPERTY_PROMPT_STORAGE_FOOTER"]             = "Storage",
    ["PROPERTY_PROMPT_WARDROBE_FOOTER"]            = "Wardrobe",
    ["REAL_ESTATE_SELL_TITLE"]                     = "Sell Property",
    ["REAL_ESTATE_SELL_DESCRIPTION"]               = "To whom would you like to sell this property?",
    ["REAL_ESTATE_SELL_ACCEPT"]                    = "ACCEPT",
    ["REAL_ESTATE_SELL_DECLINE"]                   = "DECLINE",
    ["REAL_ESTATE_SELL_TO_SELF"]                   = "You can´t sell this property to yourself, you should buy it instead.",
    ["INVALID_INPUT"]                              = "Invalid input",
    ["PLAYER_NOT_FOUND"]                           = "Player not found nearby.",
    ["PLAYER_NOT_VALID"]                           = "The selected player is not online or id is invalid.",
    ["ANOTHER_PLAYER_NEARBY"]                      = "You can´t open the storage while another player is close.",
    ["REACHED_MAX_PROPERTIES"]                     = "You have reached the maximum properties.",
    ["TARGET_REACHED_MAX_PROPERTIES"]              = "The selected user has reached the maximum properties.",
    ["NOT_ENOUGH_MONEY"]                           = "You don´t have enough to buy this property.",
    ["TARGET_NOT_ENOUGH_MONEY"]                    = "The selected user does not have enough money.",
    ["BANK_ACCOUNT_DOES_NOT_EXIST"]                = "You don´t have any bank account, it is required for paying bills.",
    ["BANK_ACCOUNT_DOES_NOT_EXIST_TARGET"]         = "The selected player does not have any bank account.",
    ["MENU_PROPERTY_TITLE"]                        = "Property ID: %s",
    ["MENU_KEYHOLDERS_LIST"]                       = "Property Keyholders",
    ["MENU_KEYHOLDERS_ADD_NEW"]                    = "Add New Keyholder",
    ["MENU_KEYHOLDERS_ADD_NEW_TITLE"]              = "Player ID",
    ["MENU_KEYHOLDERS_ADD_NEW_DESCRIPTION"]        = "To add a new property keyholder, you must insert a valid player id.",
    ["MENU_KEYHOLDERS_ADD_NEW_TO_SELF"]            = "You can´t add yourself as a keyholder, property already belongs to you.",
    ["MENU_KEYHOLDERS_REACHED_MAX"]                = "You have reached the maximum keyholders of this property.",
    ["MENU_KEYHOLDERS_ALREADY_EXISTS"]             = "The following player already exists as a keyholder.",
    ["MENU_KEYHOLDERS_DOES_NOT_EXISTS"]            = "The following player does not exist.",
    ["MENU_KEYHOLDERS_ADDED"]                      = "You have added %s as a keyholder in the property.",
    ["MENU_KEYHOLDERS_REMOVED"]                    = "You have removed %s from the property.",
    ["NOT_ALLOWED_TO_ENTER"]                       = "You are not allowed to enter this property.",
    ["MENU_TRANSFER_TITLE"]                        = "Player ID",
    ["MENU_TRANSFER_DESCRIPTION"]                  = "To add a new property owner, you must insert a valid player id.",
    ["CANNOT_TRANSFER_TO_SAME_PERSON"]             = "You can´t transfer the property to yourself while it already belongs to you.",
    ["TRANSFERRED_PROPERTY"]                       = "You have successfully transferred your property to another person.",
    ["TRANSFERRED_PROPERTY_TO_SELF"]               = "A property has been transferred to your ownership.",
    ["SUCCESSFULLY_BOUGHT_PROPERTY"]               = "You have successfully purchased a house property.",
    ["SUCCESSFULLY_SOLD_TO"]                       = "You have successfully sold the following property and received: %s Dollars",
    ["MENU_ACCEPT"]                                = "ACCEPT",
    ["MENU_DECLINE"]                               = "DECLINE",
    ["MENU_BACK"]                                  = "Go Back",
    ["MENU_WARDROBE_LOCATION"]                     = "Set Wardrobe Location",
    ["MENU_WARDROBE_LOCATION_DESCRIPTION"]         = "",
    ["MENU_STORAGE_LOCATION"]                      = "Set Storage Location",
    ["MENU_STORAGE_LOCATION_DESCRIPTION"]          = "",
    ["MENU_SET_KEYHOLDERS"]                        = "Manage Property Keyholders",
    ["MENU_SET_KEYHOLDERS_DESCRIPTION"]            = "",
    ["MENU_LEDGER"]                                = "Tax Ledger",
    ["MENU_LEDGER_HOME"]                           = "Normal Ledger",
    ["MENU_LEDGER_DESCRIPTION"]                    = "",
    ["MENU_LEDGER_SUB_DESCRIPTION"]                = "Your ledger account has: %s dollars available.",
    ["MENU_LEDGER_DEPOSIT_TITLE"]                  = "Deposit",
    ["LEDGER_TAX_SYSTEM"]                          = "Tax Payment:",
    ["LEDGER_TAX_WEEKLY_LABEL"]                    = "Days",
    ["LEDGER_TAX_MONTHLY_LABEL"]                   = "Day",
    ["LEDGER_TAX_AT"]                              = "at",
    ["MENU_LEDGER_WITHDRAW_TITLE"]                 = "Withdraw",
    ["MENU_LEDGER_WITHDRAW_DESCRIPTION"]           = "Withdraw money from the property ledger account.",
    ["MENU_TRANSFER"]                              = "Transfer",
    ["MENU_TRANSFER_DESCRIPTION"]                  = "",
    ["MENU_SELL"]                                  = "Sell",
    ["MENU_SELL_DESCRIPTION"]                      = "",
    ["MENU_SELL_ACCEPT"]                           = "Accept",
    ["MENU_SELL_ACCEPT_DESCRIPTION"]               = "Are you sure you want to sell this property?",
    ["MENU_SELL_DESCRIPTION_DOLLARS"]              = "You will receive %s dollars by placing it for sale.",
    ["MENU_SELL_DESCRIPTION_GOLD"]                 = "You will receive %s gold by placing it for sale.",
    ["SOLD_PROPERTY_RECEIVED_DOLLARS"]             = "You have sold this property and received $%s dollars.",
    ["SOLD_PROPERTY_RECEIVED_GOLD"]                = "You have sold this property and received $%s gold.",
    ["SOLD_PROPERTY"]                              = "You have sold this property.",
    ["MENU_EXIT"]                                  = "Exit Menu",
    ["SET_MENU_WARDROBE_LOCATION"]                 = "Press ~e~[G]~q~ To Set Wardrobe Location.",
    ["SET_MENU_STORAGE_LOCATION"]                  = "Press ~e~[G]~q~ To Set Storage Location.",
    ["LOCATION_SET"]                               = "The location has been set successfully.",
    ["TOO_FAR"]                                    = "You are too far from the property distance.",
    ["PROPERTY_STORAGE_TITLE"]                     = "Property Storage",
    ["PROPERTY_BILL_REASON"]                       = "Owned Property",
    ["PROPERTY_BILL_ISSUER"]                       = "Real Estate",
    ["REAL_ESTATE_SUCCESSFULLY_SOLD_DOLLARS"]      = "You have sold a property and received %s dollars.",
    ["REAL_ESTATE_SUCCESSFULLY_SOLD_GOLD"]         = "You have sold a property and received %s gold.",
    ["INPUT_DEPOSIT_TITLE"]                        = "Deposit",
    ["INPUT_DEPOSIT_DESCRIPTION"]                  = "How many would you like to deposit to the ledger?",
    ["INPUT_WITHDRAW_TITLE"]                       = "Withdraw",
    ["INPUT_WITHDRAW_DESCRIPTION"]                 = "How many would you like to withdraw from the ledger?",
    ["INPUT_ACCEPT_BUTTON"]                        = "ACCEPT",
    ["INPUT_DECLINE_BUTTON"]                       = "CANCEL",
    ["LOCKPICKING_FAILED"]                         = "The lockpick broke, progress has failed.",
    ["PROPERTY_ROBBERY_BLIP_TITLE"]                = "Property Robbery",
    ["NOT_ENOUGH_MONEY_LEDGER_DEPOSIT"]            = "You don´t have enough money to deposit.",
    ["NOT_ENOUGH_MONEY_LEDGER_WITHDRAW"]           = "You don´t have enough money to withdraw from the account.",
    ["LEDGER_DEPOSITED_ACCOUNT_MONEY"]             = "Deposited %s dollars to the ledger account.",
    ["LEDGER_WITHDREW_ACCOUNT_MONEY"]              = "Withdrew %s dollars from the ledger account.",
    ["NOT_ENOUGH_INVENTORY_WEIGHT"]                = "You don´t have enough inventory weight.",
    ["MONEY_LEDGER_DEPOSIT_LIMIT"]                 = "You are not permitted to deposit more than %s dollars.",
    ["MENU_KEYHOLDERS_MANAGE"]                     = "Would you like to manage %s?",
    ["MENU_KEYHOLDERS_PERMISSIONS_TITLE"]          = "Manage Permissions",
    ["MENU_KEYHOLDERS_REMOVE_TITLE"]               = "Remove From Keyholders",
    ["MENU_KEYHOLDERS_REMOVE_DESCRIPTION"]         = "Would you like to remove %s?",
    ["INSUFFICIENT_PERMISSIONS"]                   = "Insufficient Permissions.",
    ["PROPERTY_OWNED_KEYHOLDERS_BLIP"]             = "Guest Property",
    ["ledger_deposit"]                             = "Ledger Deposit",
    ["ledgerhome_deposit"]                         = "Home Ledger Deposit",
    ["ledgerhome_withdraw"]                        = "Home Ledger Withdraw",
    ["keyholders_management"]                      = "Keyholders Management",
    ["set_wardrobe"]                               = "Set Wardrobe",
    ["set_storage"]                                = "Set Storage",
    ["storage_access"]                             = "Storage Access",
    ["place_furniture"]                            = "Place furniture",
    ["YES"]                                        = "Has permission",
    ["NO"]                                         = "No permission",
    ["BALANCE"]                                    = "Balance",
    ["MENU_FURNITURE_SELL_DESC"]                   = "Sell your furniture for",
    ["MENU_FURNITURE_SELL_DESC_2"]                 = "of the original value",
    ["DOORLOCK"]                                   = "Lock",
    ["HOUSE"]                                      = "House",
    ["YOU_BOUGTH"]                                 = "You bought",
    ["FURNI_NOTI"]                                 = "Furniture",
    ["FURNI_PRICE"]                                = "Price:",
    ["FURNI_BUY"]                                  = "Buy",
    ["FURNI_BACK"]                                 = "Back",
    ["PROMPT_FURNI"]                               = "Open",
    ["FURNI_SHOP"]                                 = "Furniture Shop",
    ["FURNI_AMOUNT_DESC"]                          = "Select the quantity you want to buy",
    ["FURNITURE_TOO_FAR"]                          = "You are out of property range",
    ['FURNITURE_NOT_ENOUGH_WEIGHT']                = 'You are carrying too much weight!',
}